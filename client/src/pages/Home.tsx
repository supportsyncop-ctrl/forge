...
export default function Home() {
  ...
  const tasksQuery = trpc.workspace.recentTasks.useQuery();
  ...
  const sendPrompt = () => {
    ...
    createTask.mutate({ title: prompt.trim(), kind: planMode ? "plan" : "task" });
    ...
    <div className="forge-app">
      ...
      <main className="forge-main">
        ...
        <div className="forge-content">
          ...
          <section className="forge-recent"><div className="forge-command-head"><div><span className="forge-kicker">RECENT WORK</span><h2>{tasksQuery.data?.configured ? "Synced with Supabase" : "Local preview workspace"}</h2></div><span className="forge-task-count">{tasksQuery.data?.tasks.length ?? 0} TASKS</span></div><div className="forge-recent-list">{(tasksQuery.data?.tasks ?? []).slice(0, 3).map((task) => <div key={task.id}><span className="forge-recent-icon"><Clock3 size={14} /></span><span><b>{task.title}</b><small>{task.kind} · {task.status}</small></span><MoreHorizontal size={14} /></div>)}</div></section>
        </div>
        <div className="forge-composer-wrap"><div className="forge-composer"><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); sendPrompt(); } }} placeholder={`Message Forge, ${displayName}…`} rows={1} /><div className="forge-composer-bottom"><div className="forge-compose-tools"><button className="forge-tool"><TerminalSquare size={16} /> Daytona <span className={`forge-toggle tiny ${daytona ? "on" : ""}`}><i /></span></button><button className={`forge-tool forge-plan-tool ${planMode ? "selected" : ""}`} onClick={() => setPlanMode(!planMode)}><SquarePen size={15} /> Plan Mode</button><span>Enter to send · Shift + Enter for newline</span></div><button className="forge-send" onClick={sendPrompt} aria-label="Send message"><ArrowUp size={18} /></button></div></div><div className="forge-disclaimer">{createTask.isPending ? "Saving task to workspace…" : "Forge can make mistakes. Check important work before you ship it."}</div></div>
      </main>
